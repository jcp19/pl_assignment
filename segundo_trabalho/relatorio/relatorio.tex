...
\begin{itemize}
  \item o identificador é \texttt{MAP}, pelo que identifica um mapeamento. Neste primeiro caso, entra-se no estado \texttt{map}. Quando um identificador 
    ...
    as instruções que adicionam ao \textit{output} o resultado do mapeamento.
  \item o identificador é \texttt{C}, o que sinaliza uma expressão \textit{C} cujo tipo é \texttt{char *}. Neste caso, entra-se no estado
    ...
    \textit{C} e, por conseguinte, coloca-se no corpo da função a instrução para adicionar ao resultado o valor dessa expressão \textit{C};
...
Quanto mais intenso for o sinal da rede, maior frequência terão os \textit{beeps}, i.e. menor será o intervalo de tempo entre os sons.
Através do comando \texttt{iwlist scan INTERFACE}, consegue-se
...
separador que pode ser ':' ou '=', possivelmente seguido de espaços e por fim, o valor. A acção correspondente a cada um dos padrões consiste 
em retirar o nome do campo, o separador e os espaços, deixando apenas o valor, e inserir esse valor no campo correto da mensagem.
...
o \textit{template} da função \texttt{Fhtml} era inválido. Dado que este erro é crítico e impede que a função obtida possa ser compilada,
o processador de texto termina e imprime a mensagem no \textit{stderr} ``ERRO: Está a usar variáveis iguais de tipos diferentes''.
